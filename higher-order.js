...
let ordersTotal = orders.map((element, index, wholeArray) => element.price * (1 + element.tax))
...
let bobsPurchases = purchases.filter((value) => value.owner === "Bob")
bobsPrices = []
for (i = 0; i < bobsPurchases.length; i++) {
  bobsPrices.push(bobsPurchases[i].price)
}

bobsTotal = bobsPrices.reduce((total,value) => total + value)
